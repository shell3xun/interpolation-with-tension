...
spline = TensionSpline(t_obs,x_obs,sigma,'weightFunction', w_t, 'lambda',Lambda.fullTensionExpected);
...
w_r = @(z) (a1*(1+z.*z/c1).^m1 + a2*(1+z.*z/c2).^m2)./(2*(a1*m1/c1)*(1+z.*z/c1).^(m1-1) + 2*(a2*m1/c1)*(1+z.*z/c2).^(m2-1));
