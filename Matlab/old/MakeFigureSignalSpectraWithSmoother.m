...
plot(f*timescale,vmean([snn, spp],2), 'k', 'LineWidth', 2), hold on
xlog, ylog
...
for iStride=1:length(strides)
    ...
    spline_x = InterpolatingSpline(t_obs,x_obs,'S', S);
    spline_y = InterpolatingSpline(t_obs,y_obs,'S', S);
    ...
    plotHandles(1+iStride) = plot(f*timescale,RunningAverage(gamma,20)); hold on
    xlog
    xlim([min(f*timescale) max(f*timescale)])
    ylim([0 1])
...
mylegend = legend(plotHandles,'signal', sprintf('stride %d',strides(1)), sprintf('stride %d',strides(2)), sprintf('stride %d',strides(3)),'Location','southwest');
xlabel('cycles per minute', 'FontSize', figure_axis_label_size, 'FontName', figure_font);

packfig(2,1)

print('-depsc2', sprintf('../figures/interpolation_spectrum_slope%ddegree%d.eps',abs(slope),S))
return
