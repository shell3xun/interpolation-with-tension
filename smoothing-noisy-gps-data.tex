...
\cmidrule(lr){2-6} 
T & 1 & 2 & 3 & 4 & 5 \\ \midrule 
1 & 30.2-108.4\% & 27.6-105.8\% & 27.7-105.8\% & 26.9-105.7\% & 26.7-105.7\% \\ 
2 & & 1.6-15.7\% & 1.9-16.1\% & 1.8-16.1\% & 1.8-16.1\% \\ 
3 & & & 0.0-4.1\% & 0.0-4.1\% & 0.0-4.1\% \\ 
4 & & & & 0.0-3.3\% & 0.0-3.5\% \\ 
5 & & & & & 0.0-13.4\% \\ 
 \bottomrule 
\end{tabular} 
...
Is it worth comparing to cross-validation and generalized cross-validation and log-likelihood?
...
% To further improve the results, we further refine the window over which minimization is performed by attempting to deduce both $\alpha$ and the variance of $p_{\textrm{outlier}}(z)$. This leads to two equations and two unknowns,
% \begin{equation}
...
% Assume we're at full tension.
% \begin{enumerate}
%     \item Walk through possible cutoffs in $z_\textrm{crossover}$ (where the distribution switches from more likely an outlier, to characterized noise).
%     \item For a given $z_\textrm{crossover}$ you can compute the variance as a function of $\alpha$ or $\alpha$ as a function of the variance. Assume $\alpha=0$ if variance is close to expected. To good approximation then,
%     \begin{equation}
%         \sigma_o^2 = \frac{\hat{\sigma}^2 - (1-\alpha)\textrm{var}_\textrm{noise} }{3\alpha}
%     \end{equation}
%     \item For a given $z_\textrm{crossover}$ you can write the cdfs as a function of $\alpha$ and the variance.
%     \item Find the appropriate outlier solution.
%     \item Perform a KS test on all outlier solutions for which $\alpha<0.5$
% \end{enumerate}
